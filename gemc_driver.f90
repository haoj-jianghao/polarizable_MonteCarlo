...
SUBROUTINE GEMC_Driver
  ...
  USE Energy_Routines, ONLY: Compute_Total_System_Energy, System_energy_check
  USE File_Names
...
!$  time_start = omp_get_wtime()
  ...
  DO WHILE (.NOT. complete)
     ...
     rand_no = rranf()
     which_step = i
     !CALL System_Energy_Check(1,i,rand_no)
     !CALL System_Energy_Check(2,i,rand_no)
     !print *, i, rand_no    
...
!$        time_s = omp_get_wtime()
        ...
        IF(shell_mpm) THEN
           CALL Translate_MP(1,which_step)
           CALL Translate_MP(2,which_step)
        ELSE
...
!$        time_s = omp_get_wtime()
        ...
        IF(freev .GT. 1 .OR. int_sim_type == sim_gemc_npt) THEN
          ...
          CALL Volume_Change(this_box, which_step)
