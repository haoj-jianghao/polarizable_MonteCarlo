    ...
    DO ia = 1, natoms(is)
       ...
       IF ( atom_list(ia,im,is)%exist) THEN
          ...
          DO ja = ia+1,natoms(is)
             ...
             IF (atom_list(ia,im,is)%drude_type .eqv. .FALSE. .and. atom_list(ja,im,is)%drude_type .eqv. .FALSE. ) THEN         
               IF (rijsq <= rcut_lowsq) THEN
  ...
  !------------------------------------------------------------------------------------------
    ...
    ELSE IF(int_charge_sum_style(ibox) == charge_gaussian) THEN
      ...
      !  erf(alpha_ij*rij)
        ...
        IF(is .NE. js .or. im .NE. jm .or. qsc > 0.01) THEN  ! Not within the same mole or the scale factor is 1
          Eij = qi*qj/rij*(qsc - erfc(alpha_ij*rij) -erf_val)*charge_factor
        ELSE

          Eij = (qi*qj/rij)*(qsc - erf_val)*charge_factor
          !if(Eij .NE. Eij) then
          !    print *, rij, ia, im , is, ja, jm, js
          !    print *, 'Eij is not a number', Eij
          !    rij = 1d-7
          !    print *, (qi*qj/rij)*(qsc - erf_val)*charge_factor, 'qsc', qsc
          !    print *, -1.0_DP*qi*qj*alpha_ewald(ibox)*2.0/rootPI*charge_factor
          !    stop
          !end if

          if(rij <1d-6) then
                !IF(rij<1d-7)                 
                !print*, Eij, rij,-1.0_DP*qi*qj*alpha_ewald(ibox)*2.0/rootPI*charge_factor
                Eij = -1.0_DP*qi*qj*alpha_ewald(ibox)*2.0/rootPI*charge_factor
...
!    !$ include 'omp_lib.h'
  ...
  SUBROUTINE Compute_Total_System_Energy(this_box,intra_flag,overlap)
    ...
    energy(this_box)%HB_disp = 0.0_DP
...
!                rijsq = rcom * rcom
    ...
    IF(HB_term) THEN
       CALL Compute_System_HB_Body_Energy(this_box)
       energy(this_box)%total = energy(this_box)%total + energy(this_box)%HB_disp
    END IF
...
!                W_tensor_charge(:,:,this_box) = W_tensor_charge(:,:,this_box) + tc_pair(:,:)
  ...
  !------------------------------------------------------------------------------------------
    ...
    REAL(DP) :: alpha_ij, alpha_ij_constant, exp_const1, erf_val1
  !------------------------------------------------------------------------------------------
    ...
    ExistCheck: IF (atom_list(ia,im,is)%exist .AND. atom_list(ja,jm,js)%exist) THEN
       ...
       qq_calculation: IF (get_qq) THEN
          ...
          ELSEIF (int_charge_sum_style(ibox) == charge_ewald .or. int_charge_sum_style(ibox) == charge_gaussian ) THEN
             ! Real space Ewald part
             this_box = molecule_list(im,is)%which_box
             qsc = 1.0_DP
             ibox = molecule_list(im,is)%which_box 
             ...
             ELSE
               ...
               IF(rij > 1.0D-6) THEN
                 alpha_ij = alp_ij(ia,is,ja,js)
                 erf_val = 1.0_DP - erfc(alpha_ewald(ibox)*rij)
                 erf_val1 = 1.0_DP - erfc(alpha_ij*rij)
                 exp_const1 = EXP(-alpha_ij*alpha_ij*rijsq)
                 alpha_ij_constant = alpha_ij*2.0/rootPI

                 Wij_qq = qi*qj*(ewald_constant*exp_const - exp_const1*alpha_ij_constant + (erf_val1 - erf_val)/rij)
!                 Wij_qq  = qi*qj*( (qsc - erf_val)/rij + ewald_constant*exp_const )
               ELSE
                 Wij_qq = 0.0_DP
               END IF

               IF (is == js .AND. im == jm .and. qsc < 0.01) THEN
                  IF (rij > 1.0D-6) THEN
                     Wij_self = (qsc - 1.0_DP) * qi*qj * (erf_val/rij - ewald_constant * exp_const)
                     Wij_qq = Wij_qq + Wij_self
                  END IF
...
!!$    rcut_vdwsq(this_box) = rcut_vdwsq_box
  ...
  SUBROUTINE System_Energy_Check(this_box,i_step,randno)
     ...
     e_check%HB_disp    = energy(this_box)%HB_disp
     e_check%three_disp = energy(this_box)%three_disp
!print *, e_check%HB_disp
     CALL Compute_Total_System_Energy(this_box,.TRUE.,inter_overlap)
!print *, energy(this_box)%HB_disp
     ...
     e_diff%HB_disp    = ABS(energy(this_box)%HB_disp - e_check%HB_disp)
     e_diff%three_disp = ABS(energy(this_box)%three_disp - e_check%three_disp)
     ...
     IF(e_diff%HB_disp .GT. 0.000001) THEN
        WRITE(logunit,*) 'Self energy does not match', 'box: ', this_box
        WRITE(logunit,*) 'Cumulative',e_check%HB_disp,'Total',energy(this_box)%HB_disp
        aok = .FALSE.
     END IF

     IF(e_diff%three_disp .GT. 0.000001) THEN
        WRITE(logunit,*) 'Self energy does not match', 'box: ', this_box
        WRITE(logunit,*)'Cumulative',e_check%three_disp,'Total',energy(this_box)%three_disp      
         aok = .FALSE.
     END IF
          ...
          IF (get_vdw) THEN 
             ...
             IF (atom_list(ia,im_1,is_1)%exist .AND. atom_list(ja,im_2,is_2)%exist) THEN 
              ...
              IF(rij > r_born_max) THEN
                  du_dr = (B_eps_r*A_eps*dexp(-B_eps_r*rij)*rij - C_eps_r6/R6*6.0)/rijsq
            ...
            END IF
             ...
             IF (int_vdw_style(1) == vdw_lj) THEN
               ...
               ENDIF
                ...
                du_dr = 24.0_DP*eps*(2.0*SigOverR12 - SigOverR6)/Rijsq
        ...
        IF( molecule_list(this_locate,is)%which_box /= this_box ) CYCLE                   
          ...
          IF (rijsq < rcut_coulsq(this_box) ) THEN
            ...
            ELSE IF(int_charge_sum_style(this_box) == charge_gaussian) THEN
                ...
                IF (is == is1 .and. this_locate == im1 .and. qsc < 0.01) THEN
                    !if(rij>1d-6 .and. ia .NE. ia1) then
                    if(ia .NE. ia1) then        
                   
                    !print *, 'doing intra drude force cal', rij
                    ...
                    !elseif(ia .ne. ia1) then
                    !  print *, ia, ia1, 'rij=', rij
                    end if
...
!  rzijp = atom_list(ia1-1,im1,is1)%rzp - atom_list(ia1,im1,is1)%rzp
  ...
  IF( .NOT. atom_list(ia1-1,1,is1)%drude_type) THEN
   rxijp = atom_list(ia1-1,im1,is1)%rxp - atom_list(ia1,im1,is1)%rxp
   ryijp = atom_list(ia1-1,im1,is1)%ryp - atom_list(ia1,im1,is1)%ryp
   rzijp = atom_list(ia1-1,im1,is1)%rzp - atom_list(ia1,im1,is1)%rzp
  ELSE
   rxijp = atom_list(ia1-2,im1,is1)%rxp - atom_list(ia1,im1,is1)%rxp
   ryijp = atom_list(ia1-2,im1,is1)%ryp - atom_list(ia1,im1,is1)%ryp
   rzijp = atom_list(ia1-2,im1,is1)%rzp - atom_list(ia1,im1,is1)%rzp
  END IF
...
SUBROUTINE Compute_System_HB_Body_Energy(this_box)
    ...
    INTEGER :: is, js, im, jm, ibond, jbond

    INTEGER :: ia1, ia2, ia3

    INTEGER ::  this_im, this_jm

    REAL(DP) :: E_3_disp, E3, sigma_hy, eps_hy
    ...
    DO is = 1, nspecies
        ...
        IF(nhybonds(is) == 0) CYCLE

        !$OMP PARALLEL DO DEFAULT(SHARED) &     
        !$OMP PRIVATE(im, jm, ia1, ia2, ia3, sigma_hy, eps_hy) &
        !$OMP PRIVATE(ibond, this_im, this_jm, E3) &
        !$OMP SCHEDULE(STATIC) &
        !$OMP REDUCTION(+:E_3_disp) 

         DO im = 1, nmolecules(is)

          this_im = locate(im,is)
          ! is it in this box?
          IF ( molecule_list(this_im,is)%which_box /= this_box ) CYCLE
          ! is alive?
          IF ( .NOT. molecule_list(this_im,is)%live ) CYCLE

             DO ibond = 1, nhybonds(is)

               ia1 = hy_bond_list(ibond,is)%atom1
               ia2 = hy_bond_list(ibond,is)%atom2

               ia3 = hy_bond_list(ibond,is)%atom3

               sigma_hy = hy_bond_list(ibond,is)%sigma
               eps_hy   = hy_bond_list(ibond,is)%epsilonk

                IF(eps_hy > 1d-4)  THEN

                    DO jm = 1, nmolecules(is)

                     this_jm = locate(jm, is)

                     IF ( molecule_list(this_jm,is)%which_box /= this_box ) CYCLE

                     IF ( .NOT. molecule_list(this_jm,is)%live ) CYCLE

                     IF ( this_jm == this_im) CYCLE

                     CALL calculate_HB_angle(this_box, is, this_im, ia1, is, this_im , ia2, is, this_jm, ia3, sigma_hy, eps_hy, E3)

                     E_3_disp = E3 + E_3_disp
    ...
    DO is = 1, nspecies
        ...
        !$OMP PARALLEL DO DEFAULT(SHARED) &     
        !$OMP PRIVATE(im, jm, js, ia1, ia2, ia3, sigma_hy, eps_hy) &
        !$OMP PRIVATE(ibond, jbond, this_im, this_jm, E3) &
        !$OMP SCHEDULE(STATIC) &
        !$OMP REDUCTION(+:E_3_disp) 
          ...
          IF ( .NOT. molecule_list(this_im,is)%live ) CYCLE
              ...
              DO js = 1, nspecies
                 ...
                 IF(js == is) CYCLE
                      ...
                      DO jm = 1, nmolecules(js)

                           this_jm = locate(jm, js)

                           IF ( molecule_list(this_jm,js)%which_box /= this_box) CYCLE

                           IF ( .NOT. molecule_list(this_jm,js)%live ) CYCLE

                             DO ibond = 1, nhybonds(is)

                                  ia1 = hy_bond_list(ibond,is)%atom1
                                  ia2 = hy_bond_list(ibond,is)%atom2
                                  sigma_hy = hy_bond_list(ibond,is)%sigma * HB_scale_1
                                  eps_hy   = hy_bond_list(ibond,is)%epsilonk * HB_scale_2

                                    DO jbond = 1, nhybonds(js)

                                        ia3 = hy_bond_list(jbond,js)%atom3

                                        IF(eps_hy > 1d-4)  THEN

                                            CALL calculate_HB_angle(this_box, is, this_im, ia1, is, this_im, ia2, js, this_jm, ia3, sigma_hy,eps_hy, E3)
...
!
  ...
  energy(this_box)%HB_disp = E_3_disp

  !print *, E_3_disp

  END SUBROUTINE Compute_System_HB_Body_Energy


  SUBROUTINE Compute_Molecule_Nonbond_HB_Energy(this_box,im,is,E_vdw3)
    ...
    INTEGER :: this_box, im ,is
    REAL(DP):: E_vdw3

    INTEGER :: ia, js, jm, ja, ks, km, ka, start, alive1
    INTEGER :: this_jm, this_km

    INTEGER :: ia1, ia2, ia3, ibond, ia11, ia12, ia13, jbond

    REAL(DP) :: U_3_disp, E3, sigma_hy, eps_hy, sigma1_hy, eps1_hy

     E_vdw3 = 0.0_DP
     ! is it in this box?
     IF ( molecule_list(im,is)%which_box /= this_box ) Return
       ! is alive
     IF ( .NOT. molecule_list(im,is)%live ) Return

  !   nbr_bonds = bondpart_list(at1,is)%nbonds

  !   IF(nhybonds(is) == 0) RETURN
       ...
       DO ibond = 1, nhybonds(is)
           ...
           IF(eps_hy > 1d-4)  THEN
             ...
             !$OMP PARALLEL DO DEFAULT(SHARED) &     
             !$OMP PRIVATE(jm) &
             !$OMP PRIVATE(this_jm, E3) &
             !$OMP SCHEDULE(STATIC) &
             !$OMP REDUCTION(+:E_vdw3)            
               ...
               DO jm = 1 , nmolecules(is)
                 ...
                 IF (this_jm == im) CYCLE
                    ...
                    CALL calculate_HB_angle(this_box, is, im, ia1, is, im ,ia2,is, this_jm, ia3, sigma_hy, eps_hy, E3)

                    E_vdw3 = E3 + E_vdw3

                    CALL calculate_HB_angle(this_box, is, this_jm, ia1, is,this_jm ,ia2, is, im, ia3, sigma_hy, eps_hy, E3)
       ...
       DO ibond = 1, nhybonds(is)
           ...
           DO js = 1, nspecies
             ...
             !$OMP PARALLEL DO DEFAULT(SHARED) &     
             !$OMP PRIVATE(jm, jbond) &
             !$OMP PRIVATE(this_jm, E3, ia3, ia11, ia12, ia13, sigma1_hy,eps1_hy) &
             !$OMP SCHEDULE(STATIC) &
             !$OMP REDUCTION(+:E_vdw3)
                  ...
                  DO jm = 1 , nmolecules(js)
                    ...
                    DO jbond = 1, nhybonds(js)
                        ...
                        IF(eps_hy > 1d-4) THEN
                        CALL calculate_HB_angle(this_box, is, im, ia1, is, im,ia2, js, this_jm, ia3, sigma_hy, eps_hy, E3)
                        E_vdw3 = E3 + E_vdw3
                        END IF

                        ia11 = hy_bond_list(jbond,js)%atom1
                        ia12 = hy_bond_list(jbond,js)%atom2
                        ia13 = hy_bond_list(ibond,is)%atom3

                        sigma1_hy = hy_bond_list(jbond,js)%sigma * HB_scale_1
                        eps1_hy   = hy_bond_list(jbond,js)%epsilonk * HB_scale_2

                        IF(eps1_hy > 1d-4) THEN
                        !print *, 'run'
                        CALL calculate_HB_angle(this_box, js, this_jm, ia11,js, this_jm ,ia12, is, im, ia13, sigma1_hy, eps1_hy, E3)
                        E_vdw3 = E3 + E_vdw3
                        END IF
  ...
  END SUBROUTINE Compute_Molecule_Nonbond_HB_Energy



  SUBROUTINE calculate_HB_angle(this_box, is1, im1, ia1, is2, im2, ia2, is3, im3, ia3, sigma, eps,  E3)

  ! this_box, is, this_im, ia1, is, this_im , ia2, is, this_jm, ia3, sigma_hy,
  ! eps_hy, E3
  ...
  IMPLICIT NONE
  INTEGER  :: this_box
  INTEGER  :: is1,is2,is3,im1,im2,im3,ia1,ia2,ia3
  REAL(DP) :: sigma, eps
  ...
  REAL(DP) :: rcub
  REAL(DP) :: r1, r2, r3
  REAL(DP) :: cos_the1, cos_the2, cos_the3
  ...
  rxijp = atom_list(ia2,im2,is2)%rxp - atom_list(ia1,im1,is1)%rxp
  ryijp = atom_list(ia2,im2,is2)%ryp - atom_list(ia1,im1,is1)%ryp
  rzijp = atom_list(ia2,im2,is2)%rzp - atom_list(ia1,im1,is1)%rzp

          ! Now get the minimum image separation 
  CALL Minimum_Image_Separation(this_box,rxijp,ryijp,rzijp,rxij,ryij,rzij)

  U1(1:3) = (/rxij, ryij, rzij/)  !vector 1 - 2
  r1 = rxij*rxij + ryij*ryij + rzij*rzij

  rxijp = atom_list(ia2,im2,is2)%rxp - atom_list(ia3,im3,is3)%rxp
  ryijp = atom_list(ia2,im2,is2)%ryp - atom_list(ia3,im3,is3)%ryp
  rzijp = atom_list(ia2,im2,is2)%rzp - atom_list(ia3,im3,is3)%rzp

          ! Now get the minimum image separation 
  CALL Minimum_Image_Separation(this_box,rxijp,ryijp,rzijp,rxij,ryij,rzij)

  U2(1:3) = (/rxij, ryij, rzij/)  !vector 1 - 3
  r2 = rxij*rxij + ryij*ryij + rzij*rzij


  rxijp = atom_list(ia1,im1,is1)%rxp - atom_list(ia3,im3,is3)%rxp
  ryijp = atom_list(ia1,im1,is1)%ryp - atom_list(ia3,im3,is3)%ryp
  rzijp = atom_list(ia1,im1,is1)%rzp - atom_list(ia3,im3,is3)%rzp

          ! Now get the minimum image separation 
  CALL Minimum_Image_Separation(this_box,rxijp,ryijp,rzijp,rxij,ryij,rzij)

  U3(1:3) = (/rxij, ryij, rzij/)  !vector 2 - 3
  r3 = rxij*rxij + ryij*ryij + rzij*rzij

  IF (r3 > 9.0*9.0) RETURN

  cos_the1 = DOT_PRODUCT(U1, U2)/sqrt(r1*r2)

  IF (cos_the1 > 0) RETURN
  ...
  E3 = eps*(5.0*r3 - 6.0*r3/r_sig) * (cos_the1 * cos_the1)**2.0

  RETURN

  END SUBROUTINE calculate_HB_angle
