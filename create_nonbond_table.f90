       ...
       DO jtype = 1, nbr_atomtypes
          ...
          ! LB mixing rule: epsij = (epsi * epsj)^(1/2); sigmaij = 1/2 (sigmai + sigmaj)
                IF (mix_rule == 'LB') THEN
                     ...
                     IF(atom_type_list(itype) == 'O1' .or. atom_type_list(jtype) == 'O1') THEN
                          IF(atom_type_list(itype) == 'O1'.and. atom_type_list(jtype) == 'O1') THEN
                          vdw_param1_table(itype,jtype) =vdw_param1_table(itype,jtype)
                          ELSE IF(atom_type_list(itype) == 'C1'.or. atom_type_list(jtype) == 'C1') THEN
                          vdw_param1_table(itype,jtype) =vdw_param1_table(itype,jtype) * (1.0_DP + binary_kij_3)
                          print *,vdw_param1_table(itype,jtype),atom_type_list(itype) , atom_type_list(jtype)
                          ELSE
                          vdw_param1_table(itype,jtype) =vdw_param1_table(itype,jtype) * (1.0_DP + binary_kij_4)
                          print *,vdw_param1_table(itype,jtype),atom_type_list(itype) , atom_type_list(jtype)
                          END IF
                     END IF
                END IF
             ...
             ELSE
                ...
                IF (mix_rule == 'LB') THEN
                     ...
                     IF(atom_type_list(itype) == 'O1' .or. atom_type_list(jtype) == 'O1') THEN
                          ...
                          ELSE IF(atom_type_list(itype) == 'C1' .or. atom_type_list(jtype) == 'C1') THEN
                          vdw_param2_table(itype,jtype) = vdw_param2_table(itype,jtype) * (1.0_DP + binary_kij_1)
                          print *, vdw_param2_table(itype,jtype),atom_type_list(itype) , atom_type_list(jtype)
                          ELSE
                          vdw_param2_table(itype,jtype) = vdw_param2_table(itype,jtype) * (1.0_DP + binary_kij_2)
                          print *, vdw_param2_table(itype,jtype),atom_type_list(itype) , atom_type_list(jtype)
                          END IF
                     END IF
          ...
          ! geometric mixing rule: epsij = (epsi * epsj)^(1/2); sigmaij = (sigmai * sigmaj)^(1/2)
                ...
                IF (mix_rule == 'kong')  THEN
                     ...
                     print *, atom_type_list(itype), atom_type_list(jtype), vdw_param1_table(itype,jtype)

                     IF(atom_type_list(itype) == 'O1' .or. atom_type_list(jtype) == 'O1') THEN
                          IF(atom_type_list(itype) == 'O1' .and. atom_type_list(jtype) == 'O1') THEN
                          vdw_param1_table(itype,jtype) = vdw_param1_table(itype,jtype) 
                          ELSE IF(atom_type_list(itype) == 'C1' .or. atom_type_list(jtype) == 'C1') THEN
                          vdw_param1_table(itype,jtype) =  vdw_param1_table(itype,jtype) * (1.0_DP + binary_kij_1)
                          print *, vdw_param1_table(itype,jtype), atom_type_list(itype) , atom_type_list(jtype)
                          ELSE
                          vdw_param1_table(itype,jtype) =  vdw_param1_table(itype,jtype) * (1.0_DP + binary_kij_2)
                          print *, vdw_param1_table(itype,jtype), atom_type_list(itype) , atom_type_list(jtype) 
                          END IF
                     END IF
             ...
             ENDIF
              ...
              ELSE
                ...
                IF (mix_rule == 'geometric')  &
                     vdw_param2_table(itype,jtype) = dsqrt(temp_param_i(2)*temp_param_j(2))                
                IF (mix_rule == 'kong') &
                    ! vdw_param2_table(itype,jtype) = (temp_param_i(2)+temp_param_j(2))/2.0 !LB for B parameter 

                    vdw_param2_table(itype,jtype) = 2.0*(temp_param_i(2)*temp_param_j(2)) / (temp_param_i(2)+temp_param_j(2)) ! Kong for B parameter                  

                     IF(atom_type_list(itype) == 'O1' .or.atom_type_list(jtype)== 'O1') THEN
                          IF(atom_type_list(itype) == 'O1' .and.atom_type_list(jtype) == 'O1') THEN
                          vdw_param2_table(itype,jtype) = vdw_param2_table(itype,jtype)
                          ELSE IF(atom_type_list(itype) == 'C1' .or. atom_type_list(jtype) == 'C1') THEN
                          vdw_param2_table(itype,jtype) = 2.0*(temp_param_i(2)*temp_param_j(2)) / (temp_param_i(2)+temp_param_j(2)) * (1.0_DP + binary_kij_5)
                          print *, vdw_param2_table(itype,jtype), atom_type_list(itype) , atom_type_list(jtype)
                          ELSE
                          vdw_param2_table(itype,jtype) = 2.0*(temp_param_i(2)*temp_param_j(2)) / (temp_param_i(2)+temp_param_j(2)) * (1.0_DP + binary_kij_6)
                          print *, vdw_param2_table(itype,jtype), atom_type_list(itype) , atom_type_list(jtype)
                          END IF
                    END IF
              ...
              ENDIF
                ...
                IF (mix_rule == 'kong')  THEN
                     ...
                     IF(atom_type_list(itype) == 'O1' .or. atom_type_list(jtype)== 'O1') THEN
                          IF(atom_type_list(itype) == 'O1' .and. atom_type_list(jtype) == 'O1') THEN
                          vdw_param3_table(itype,jtype) = vdw_param3_table(itype,jtype)
                          ELSE IF(atom_type_list(itype) == 'C1' .or. atom_type_list(jtype) == 'C1') THEN
                          vdw_param3_table(itype,jtype) = dsqrt(temp_param_i(3)*temp_param_j(3)) * (1.0_DP + binary_kij_3)
                          print *, vdw_param3_table(itype,jtype), atom_type_list(itype) , atom_type_list(jtype)
                          ELSE
                          vdw_param3_table(itype,jtype) = dsqrt(temp_param_i(3)*temp_param_j(3)) * (1.0_DP + binary_kij_4)
                          print *, vdw_param3_table(itype,jtype), atom_type_list(itype) , atom_type_list(jtype)
                          END IF
                    END IF
                 ENDIF
         ...
         IF (int_vdw_style(1) == born_cut_tail) THEN
          ...
          WRITE(logunit,'(A6,5x,A6,2x,T20,f16.6,T50,f16.6)') &
               atom_type_list(itype), atom_type_list(jtype), &
