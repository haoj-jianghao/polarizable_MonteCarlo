 ...
 ! print *, frag_order 
  ...
  END IF
   ...
   !$OMP PARALLEL WORKSHARE DEFAULT(SHARED)
    cos_sum_old(:,box_in) = cos_sum(:,box_in)
    sin_sum_old(:,box_in) = sin_sum(:,box_in)
   ...
   IF(species_list(this_species)%int_insert == int_random) THEN
     ...
     !print *, 'calculate angle', this_species, e_angle_in
   ...
   !if(this_species==1) print *, species_list(this_species)%int_insert == int_random, E_angle_in
  ...
  new_atom_list(1:natoms(this_species)) = atom_list(1:natoms(this_species),alive,this_species)
...
!     DEALLOCATE(frag_order)
     ...
     IF (cbmc_overlap) THEN
       ...
       !$OMP PARALLEL WORKSHARE DEFAULT(SHARED)
        cos_sum(:,box_in) = cos_sum_old(:,box_in)
        sin_sum(:,box_in) = sin_sum_old(:,box_in)
       !$OMP END PARALLEL WORKSHARE
  ...
  END IF
   ...
   conv_drude2 = .TRUE.
    ...
    !$OMP PARALLEL WORKSHARE DEFAULT(SHARED)
      cos_sum_old(:,box_out) = cos_sum(:,box_out)
      sin_sum_old(:,box_out) = sin_sum(:,box_out)
    !$OMP END PARALLEL WORKSHARE
  ...
  factor = beta(box_in)*(energy(box_in)%total - energy_old_in%total - e_angle_in - nrg_ring_frag_in)  + beta(box_out)*(energy(box_out)%total - energy_old_out%total-e_angle_out - nrg_ring_frag_out )
