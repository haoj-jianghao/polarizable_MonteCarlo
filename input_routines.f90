...
! No name specified so abort
  ...
  ALLOCATE( nhybonds(nspecies),Stat = AllocateStatus )
  IF (AllocateStatus /= 0) THEN
     write(*,*)'memory could no tbe allocated for nhybonds array'
     write(*,*)'stopping'
     STOP
  END IF
...
! Assign the first entry on the line to the mixing rule
        ...
        ENDIF
           ...
           binary_kij_1 = string_to_Double(line_array(2))
           binary_kij_2 = string_to_Double(line_array(3))
           binary_kij_3 = string_to_Double(line_array(4))
           binary_kij_4 = string_to_Double(line_array(5))
           binary_kij_5 = string_to_Double(line_array(6))
           binary_kij_6 = string_to_Double(line_array(7))
           ...
           species_loop:DO i=1,nspecies
              ...
              mcf_read_loop:DO 
                 ...
                 IF (ierr == 0) THEN
                    ...
                    ELSEIF (line_string(1:20) == '# Hydrogen_Bond_Info') THEN
                       CALL Read_String(molfile_unit,line_string,ierr)
                       nhybonds(i) = String_To_Int(line_string)
                       IF(nhybonds(i) > 0) HB_term = .TRUE.
                       WRITE(logunit,*) '  ', &
                            TRIM(Int_To_String(nhybonds(i))), 'hydrogen bond(s)specified.'

                       WRITE(logunit,*) 'hydrogen bond term', HB_term
...
! No connectivity file specified so abort
  ...
  ALLOCATE(hy_bond_list(MAXVAL(nhybonds), nspecies), Stat = AllocateStatus )
  IF (AllocateStatus /= 0) THEN
     write(*,*)'memory could no tbe allocated for hydrogen_ bond_list array'
     write(*,*)'stopping'
     STOP
  END IF
...
SUBROUTINE Get_HydrogenBond_Info(is)
!********************************************************************************
  ! This routine opens the molfile and loads information into the bond_list
  ! for species is.
  ! Written by: E. Maginn
...
!********************************************************************************
  ...
  DO
     ...
     IF (line_string(1:20) == '# Hydrogen_Bond_Info') THEN
        line_nbr = line_nbr + 1

        line_array = ""
        CALL Parse_String(molfile_unit,line_nbr,1,nbr_entries,line_array,ierr)
        ! Make sure the number of bonds is still the same. It should not have
        ! changed!
        IF (String_To_Int(line_array(1)) /= nhybonds(is)) THEN
           err_msg = ""
           err_msg(1) = 'nbonds is inconsistent'
           CALL Clean_Abort(err_msg,'Get_HydrogenBond_Info')
        ENDIF

        IF (nhybonds(is) == 0) THEN
           WRITE(logunit,*) 'No hydrogen bonds in species ',is
           EXIT
        ENDIF

        DO ib = 1,nhybonds(is)
           ! Now read the entries on the next lines. There must be at least 4
           ! for 
           ! each bond.
           line_array = ""
           CALL Parse_String(molfile_unit,line_nbr,10,nbr_entries,line_array,ierr)
           ...
           ! Assign appropriate values to list elements
           hy_bond_list(ib,is)%atom1 = String_To_Int(line_array(2))  !lone pair oxygen (donor)
           hy_bond_list(ib,is)%atom2 = String_To_Int(line_array(3))  !lone pair hydrogen (cetral atom)
           hy_bond_list(ib,is)%atom3 = String_To_Int(line_array(4))  !acceptor

           hy_bond_list(ib,is)%sigma = string_To_Double(line_array(5))
           hy_bond_list(ib,is)%epsilonk = string_To_Double(line_array(6))*kboltz

           HB_scale_1 = string_To_Double(line_array(9))
           HB_scale_2 = string_To_Double(line_array(10))
           
          ! IF (verbose_log) THEN
                   WRITE(logunit,'(A,T25,I3,1x,I3)') 'Species and hydrogen bondnumber', is,ib
                   WRITE(logunit,'(A,T25,I3)') 'donor:',hy_bond_list(ib,is)%atom1
                   WRITE(logunit,'(A,T25,I3)') 'centralatom:',hy_bond_list(ib,is)%atom2
                   WRITE(logunit,'(A,T25,I3)')'acceptor:',hy_bond_list(ib,is)%atom3
                   WRITE(logunit,'(A,T25,F10.4)')'sigma:',hy_bond_list(ib,is)%sigma
                   WRITE(logunit,'(A,T25,F10.4)')'epsilonk:',hy_bond_list(ib,is)%epsilonk/kboltz

                   WRITE(logunit, *) 'HB between CO2 and H2O increased', HB_scale_1, HB_scale_2
         !  END IF
     ...
     ELSEIF (line_string(1:3) == 'END' .or. line_nbr > 10000) THEN
        ...
        ! Problem reading Bond_Info
        err_msg = ""
        err_msg(1) = 'Trouble locating # HydorgenBond_Info keyword'
        CALL Clean_Abort(err_msg,'Get_HydrogenBond_Info')
  ...
  WRITE(logunit,*) '*** Completed assigning Hydrogen bond info ***'
  WRITE(logunit,*)

END SUBROUTINE Get_HydrogenBond_Info
