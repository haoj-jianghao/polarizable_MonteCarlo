...
!  REAL(dp) :: dispp_max
  ...
  energy_cal =.FALSE.
   ...
   DO is = 1, nspecies
      ...
      DO im = 1, nmolecules(is)    !nmols(is,this_box)           
        ...
        call Compute_Molecule_Nonbond_Inter_Force(alive, is, Fxx(:, alive, is), Fyy(:, alive, is), Fzz(:, alive, is), &
  ...
  ! dispp_max is an adjustable parameter for multiparticle move
   ...
   LOOP1:DO is = 1, nspecies
      ...
      LOOP2:DO im = 1, nmolecules(is) !nmols(is,this_box)     
        ...
        dmpmx(alive,is) = beta(this_box)*dispp_max*sum(Fxx(:, alive, is)) + gamma_x_a
        dmpmy(alive,is) = beta(this_box)*dispp_max*sum(Fyy(:, alive, is)) + gamma_y_a
        dmpmz(alive,is) = beta(this_box)*dispp_max*sum(Fzz(:, alive, is)) + gamma_z_a
        atom_list(:,alive,is)%rxp = atom_list(:,alive,is)%rxp + dmpmx(alive, is)
        atom_list(:,alive,is)%ryp = atom_list(:,alive,is)%ryp + dmpmy(alive, is)
        atom_list(:,alive,is)%rzp = atom_list(:,alive,is)%rzp + dmpmz(alive, is)
        molecule_list(alive,is)%xcom = molecule_list(alive,is)%xcom + dmpmx(alive, is)
        molecule_list(alive,is)%ycom = molecule_list(alive,is)%ycom + dmpmy(alive, is)
        molecule_list(alive,is)%zcom = molecule_list(alive,is)%zcom + dmpmz(alive, is)
        S_is = S_is + (beta(this_box)*dispp_max*sum(Fxx(:, alive, is)) - dmpmx(alive,is))*&
                                (beta(this_box)*dispp_max*sum(Fxx(:, alive, is)) - dmpmx(alive,is))
        S_is = S_is + (beta(this_box)*dispp_max*sum(Fyy(:, alive, is)) - dmpmy(alive,is))*&
                                (beta(this_box)*dispp_max*sum(Fyy(:, alive, is)) - dmpmy(alive,is))
        S_is = S_is + (beta(this_box)*dispp_max*sum(Fzz(:, alive, is)) - dmpmz(alive,is))*&
                                (beta(this_box)*dispp_max*sum(Fzz(:, alive, is )) - dmpmz(alive,is))
  ...
  cos_sum_old(:,this_box) = cos_sum(:,this_box)
  sin_sum_old(:,this_box) = sin_sum(:,this_box)
  ...
  E_HB_move = energy(this_box)%HB_disp
  END IF

  delta_e = ( E_vdw_move/2.0 - E_vdw ) + ( E_qq_move/2.0 - E_qq ) + E_HB_move - E_HB
  ...
  ELSE
   ...
   IF(HB_term) energy(this_box)%HB_disp  = E_HB
 ...
 DO is = 1, nspecies
     IF ( MOD(ntrials(is,this_box)%displacement,nupdate) == 0 .and. ntrials(is,this_box)%displacement .NE. 0  ) THEN
     ...
     END IF
     WRITE(logunit,*)
!     print *, success_ratio, nequil_success(is,this_box)%displacement, nupdate, max_disp(is,this_box)
     ...
     LOOP32:  DO im = 1, nmolecules(is) ! nmols(is,this_box)  
      ...
      if(.NOT. molecule_list(alive,is)%live) cycle
       ...
       IF (inter_overlap) THEN
          ...
          STOP
       END IF    
       CALL Compute_Molecule_Torque(alive, is, this_box, Fxx(:, alive, is), Fyy(:, alive, is), Fzz(:, alive, is), Mxx(alive, is), Myy(alive, is), Mzz(alive, is) )          
     END DO LOOP32     
     !$OMP END PARALLEL DO
   END DO LOOP31
    ...
    E_bond = energy(this_box)%bond
    E_intra = energy(this_box)%intra
    IF(Hb_term) E_HB = energy(this_box)%HB_disp
...
!     if(is .gt. 2) cycle
     LOOP34: DO im = 1, nmolecules(is)  !nmols(is,this_box)
        ...
        gamma_y_a = g_random(mean_a, dev_b) 
        gamma_z_a = g_random(mean_a, dev_b)
        thetax(alive,is) =  beta(this_box)*rott_max*Mxx(alive, is) + gamma_x_a
        thetay(alive,is) =  beta(this_box)*rott_max*Myy(alive, is) + gamma_y_a
        thetaz(alive,is) =  beta(this_box)*rott_max*Mzz(alive, is) + gamma_z_a
        S_is = S_is + (beta(this_box)*rott_max*Mxx(alive,is) - thetax(alive,is))*&
                                 (beta(this_box)*rott_max*Mxx(alive,is) - thetax(alive,is)) 
        S_is = S_is + (beta(this_box)*rott_max*Myy(alive,is) - thetay(alive,is))*&
                                (beta(this_box)*rott_max*Myy(alive,is) - thetay(alive,is)) 
        S_is = S_is + (beta(this_box)*rott_max*Mzz(alive,is) - thetaz(alive,is))*&
                                (beta(this_box)*rott_max*Mzz(alive,is) - thetaz(alive,is)) 
        CALL Rotate_Molecule_Axis_MPM(alive, is,  thetax(alive,is), thetay(alive,is), thetaz(alive,is)) !Rigid rotate molecule around COM, the rotation angle is biased by the torque on the molecule
      END DO LOOP34
