...
SUBROUTINE Widom_Chempot(this_box,is)
 ...
 !IF(is .NE. 2) then
 !  print *, 'Currently only support binary system with species 2 as ghost particle'
 !  stop
 !end if
 ...
 IF ( locate(nmolecules(is)+1,is) == 0 ) THEN
  ...
  !ELSE
     ...
     atom_list(:,alive,is)%rxp =   frag_coords(:,1,frag_list(1,is)%type)%rxp
     atom_list(:,alive,is)%ryp =   frag_coords(:,1,frag_list(1,is)%type)%ryp !init_list(:,1,is)%ryp
     atom_list(:,alive,is)%rzp =   frag_coords(:,1,frag_list(1,is)%type)%rzp !init_list(:,1,is)%rzp
  ...
  !END IF
     ...
     print *, atom_list(1:5,alive,is)%rxp, atom_list(1:5,alive,is)%ryp
